...
In-paper anchor: `paper/GREEN_GRIFFITHS_LANG_CONJECTURE_PREPRINT.md` (`GGL_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving algebraic degeneracy of entire curves on varieties of general type through an admissible hyperbolic closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`GGL_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and admissible windows do not collapse.
This is the first-failure capture contribution to `M_GGL`.
...
If near-failure extraction and no-Zeno spacing hold, then `GGL_G3` prevents hidden drift before rigidity is applied.
