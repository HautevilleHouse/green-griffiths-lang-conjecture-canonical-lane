...
In-paper anchor: `paper/GREEN_GRIFFITHS_LANG_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving algebraic degeneracy of entire curves on varieties of general type through an admissible hyperbolic closure architecture`.
...
2. the endpoint-transfer lock `jet_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_GGL` remains positive.
...
Payload: verify `jet_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_GGL` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `jet_transfer`
